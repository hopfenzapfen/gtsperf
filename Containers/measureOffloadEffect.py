...
Usage: python measureOffloadEffect.py <overlay-name> <iperf3-server-IP> <interface>
Description: This script measures the TCP throughput with and without NIC offload.
             It specifically enables and disables "General segmentation offloading" and
             "TCP segmentation offloading".
             The measured results and the resulting means and variances are stored
             in the "result" folder created by this script.
...
def runTests(testRepetitions, repInterval, dataDir, overlayNetwork, interface,
             serverAddr, duration):
    """
    Measures the TCP with and without TCP segmentation offloading.
